...
try {
    if (isset($_POST['apellidoMaterno']) && isset($_POST['apellidoPaterno']) && isset($_POST['codigo']) && isset($_POST['condicionSunat']) && isset($_POST['docIdentidad']) && isset($_POST['estadoSunat']) &&  isset($_POST['nombre']) && isset($_POST['telefono'])) {
        ...
        if ($_POST['codigo'] == "") {
            ...
            echo json_encode($mensaje);
        } else  if (exixtenciaProveedor($_POST['docIdentidad']) == 1 ) {
            $mensaje  = array(
                "warning" => "false",
                "msj" => "El proveedor ya se encuentra registrado",
                "color" => "linear-gradient(to right, #ff5c74,  #e30e2e )"
            );
            echo json_encode($mensaje);
            ...
            $mensaje  = array(
                "warning" => "false",
                "msj" => "Asigne un nombre al proveedor",
                "color" => "linear-gradient(to right, #ff5c74,  #e30e2e )"
            );
            echo json_encode($mensaje);
        } else {
        ...
        $telefono = $_POST['telefono'];
            ...
            guardarProveedor($codigo,$nombre,$apellidoPaterno,$apellidoMaterno,$docIdentidad,$direccion,$telefono,$estadoSunat,$condicionSunat);
